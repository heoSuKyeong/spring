	...
	<script>
		...
		function connect(name) {
			...
			ws.onopen = function(evt) {
				...
				//서버와 연결 직 후 본인의 정보를 서버에 전달한다.
				//사람이 들어온 상태를 전달하고 메시지도 ws.send로 전달한다.
				//어떤 용도의 메시지인지 서버측에서는 알기 힘들기 때문에 메시지의 규칙으로 구분지어 전송한다.
				//메시지의 규칙으로 숫자는 상태코드를 나타내고 그 상태코드에 따라 뒷 내용을 구분 짓는다.
				//이런 메시지의 규칙(집합)을 프로토콜이라고 한다.
				//ws.send('1|홍길동');	1|본인대화명
				//ws.send('2|1|2|점메추');	2|보내는사람|받는사람|대화메시지
				
				/*
				위와 같은 csv는 작성하기 힘들다.
				메시지 규칙
				메시지를 JSON으로 만들어 서버에게 전송한다.
		...
		//접속 종료하는 소켓이 없으므로 창을 닫았을 때, 종료했다고 가정
		//창이 닫히기 바로 직전에 발생
